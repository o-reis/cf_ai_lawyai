...
if (!CF_TOKEN || !ACCOUNT_ID || !DB_ID) {
  console.error(
    "Missing credentials. Make sure USER_API, USER_ID and DB_ID are set in .env"
  );
  process.exit(1);
}
...
async function embedTexts(texts) {
  ...
  const json = await res.json();
  if (!json.success)
    throw new Error("AI embed failed: " + JSON.stringify(json.errors));
...
async function main() {
  ...
  console.log(
    `D1 batch: ${D1_BATCH} | Embed sub-batch: ${EMBED_BATCH} | Starting offset: ${offset}\n`
  );
  ...
  while (true) {
    ...
    try {
      ...
      console.error(
        `Resume with: OFFSET_START=${offset} node index-articles.js`
      );
      process.exit(1);
    }
    ...
    for (let i = 0; i < articles.length; i += EMBED_BATCH) {
      ...
      try {
        ...
        console.error(
          `\nEmbed failed at offset=${offset} sub-batch i=${i}:`,
          err.message
        );
        console.error(
          `Resume with: OFFSET_START=${offset} node index-articles.js`
